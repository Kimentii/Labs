...
void Fill(it first, it last, T val);			//Все значения элементов в интервале заменяет на val
template <class it, class T>
it Find(it first, it last, const T& val);		//Возвращает итератор на элемент с значенеием val
template <class it, class T>
size_t Count(it first, it last, T val);			//Подсчитывает количество элементов с значением val