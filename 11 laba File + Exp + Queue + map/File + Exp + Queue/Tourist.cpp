#include "Tourist.h"
Tourist::Tourist(const char* n, const char* sn, const char* mn, int y, int p) : Human(n, sn, mn, y)
{																			//Конструктор
	for (int i = 0; i < NUM; i++)
	{
...
fstream& operator << (fstream& os, const Tourist& aT)						//Перегрузка записи в файл
{
	os << dynamic_cast<const Human&>(aT);
	for (int i = 0; i < NUM; i++)											//Запись путешествий
	{
		os << ' ' << aT.m_Travels[i].Country;
		os << ' ' << aT.m_Travels[i].Date;
	}
	os << endl;
	return os;
}
fstream& operator >> (fstream& is, Tourist& aTur)							//Перегрузка чтения из файла
{
	is >> dynamic_cast<Human&>(aTur);
	for (int i = 0; i < NUM; i++)											//Чтение путешествий
	{
